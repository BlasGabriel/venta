...
export const listar = async (req, res, next) => {
  try {
    ...
    // Itera sobre cada cuenta por pagar
    const cuentasConPagosCompletados = cobros.map((cuenta) => {
      // Verifica si la cantidad de item_cobro coincide con la cantidad de item_cobro con es_cobrado=1
      const pagosCompletados = cuenta.item_cobro.filter(
        (pago) => pago.es_cobrado == 1
      ).length;
      // Asigna 1 si los pagos completados son iguales a la cantidad total de pagos, de lo contrario asigna 0
      cuenta.pagos_completados = pagosCompletados == cuenta.item_cobro.length ? 1 : 0;
      cuenta.por_pagar = cuenta.item_cobro.length - pagosCompletados;
      cuenta.cuentas = cuenta.item_cobro.length;
      return cuenta;
    });
    res.json(cuentasConPagosCompletados);
  } catch (error) {
    next(error);
  }
};
